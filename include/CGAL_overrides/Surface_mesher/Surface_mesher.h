...
//
// $URL: svn+ssh://scm.gforge.inria.fr/svn/cgal/branches/next/Surface_mesher/include/CGAL/Surface_mesher/Surface_mesher.h $
...
// --- Added: Ricard Campos (Start) ---
#include <boost/filesystem.hpp>
// --- Added: Ricard Campos  (End)  ---
  ...
  public:
    ...
    void refine_mesh(Visitor visitor) {
      ...
      if (verbose == NOT_VERBOSE)
		  ...
		  // Ricard Campos: Modified debug output
		  std::cerr << "Refining...\n";
		  int nbsteps = 0;
		  CGAL::Timer timer;
          std::cerr << "Legende of the following line: "
                    ...
                    << ")\n";
		  std::cerr << boost::format("\r             \r" "(%1%,%2%,%3%) (%|4$.1f| vertices/s)")
					   % tr.number_of_vertices()
					   % nbsteps % this->debug_info()
					   % (nbsteps / timer.time());

#ifdef SURFACE_MESHER_DEBUG_SAVE_INTERMEDIATE
		  // Creating debug output path
		  ...
		  oss0 << "./_OUT/Surface_mesher_debug_ " << nbsteps << ".off" ;
		  std::ofstream out0( oss0.str().c_str() ) ; 
		  CGAL::output_surface_facets_to_off( out0, c2t3 ) ; 
		  out0.close() ;
#endif
		  ...
		  while (!is_algorithm_done()) {
			...
			if ( ( nbsteps % 1000 ) == 0 ) {
				...
				oss << "./_OUT/Surface_mesher_debug_ " << nbsteps << ".off" ;
