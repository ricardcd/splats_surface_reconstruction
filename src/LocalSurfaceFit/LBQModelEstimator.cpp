...
LBQModelEstimator::LBQModelEstimator( const FT &distThres ) : ModelEstimator< Point_3, FT >( 6 ), 
															  m_sqDistanceThreshold( distThres*distThres ){}


void LBQModelEstimator::fit(	const std::vector< typename std::vector< Point_3 >::const_pointer > &data,
...
void LBQModelEstimator::leastSquaresFit( const std::vector< typename std::vector< Point_3 >::const_pointer > &data,
								  std::vector< FT > &parameters ) const {

	// Fitting is already a least squares process...
	fit( data, parameters ) ;	
	//std::cout << "Least squares parameters" << std::endl ;
...
bool LBQModelEstimator::compatible(	const std::vector< double > &parameters, 
...
std::vector< double > LBQModelEstimator::sqResiduals( const std::vector< FT > &parameters, 
	...
	for( it = data.begin(); it != data.end(); ++it ) {
		...
		sqResiduals.push_back( sqDistance ) ;
	}
...
bool LBQModelEstimator::degenerate( const std::vector< std::vector< Point_3 >::const_pointer > &data ) const {
...
LBQModelEstimator::LocalBivariateQuadric	LBQModelEstimator::parameters2LBQ( const std::vector< FT > &parameters ) {
...
std::vector< FT > LBQModelEstimator::LBQ2parameters( const LocalBivariateQuadric& lbq ) {
